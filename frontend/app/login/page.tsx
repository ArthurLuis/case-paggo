...
const loginUser = async (email: string, password: string) => {
  ...
  console.log('Chamando API em: ', loginUrl);
  try {
    const response = await axios.post(loginUrl, {email, password});

    if (response.status === 200) {
      // Armazenando o token JWT no cookie diretamente no lado do cliente
      // Definir o cookie (configuração de expiração de 1 dia)
      document.cookie = `authToken=${response.data.access_token}; max-age=86400; path=/; Secure; SameSite=Strict`; // Adiciona o cookie
...
const Login = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (token) {
        // Garantir que o token foi armazenado corretamente
        ...
        console.log('Token armazenado nos cookies:', cookies.authToken);
